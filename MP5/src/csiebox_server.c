...
void sig_handler(int signo) 
{
  if (signo == SIGINT || signo == SIGTERM) 
  {
    ...
    if (unlink(TARGET_FIFO) != 0) {
      perror("unlink fifo fail\n");
    }
    else {
      // fprintf(stderr, "delete fifo: %s\n", TARGET_FIFO);
    }
  ...
  {
    //TODO//
    uint32_t thread_num_sent = htonl(active_thread_num);
    int fifofd = open(TARGET_FIFO, O_WRONLY);

    if (fifofd < 0) 
      perror("open fifo fail\n");
      
    else {
      write(fifofd, &thread_num_sent, sizeof(uint32_t));
      close(fifofd);
    }
  }
}
...
void csiebox_server_init(csiebox_server** server, int argc, char** argv) {
  ...
  *server = tmp;
  ...
  if (tmp->arg.daemonize)
  {
    ...
    if (chdir(tmp->arg.run_path) < 0)
      fprintf(stderr, "Cannot switch to tmp directory\n");

    // rewrite csiebox_server.pid
    int pidfd = open("csiebox_server.pid", O_WRONLY);
    char pid_string[8] = {0};
    sprintf(pid_string, "%d", getpid());
    write(pidfd, pid_string, strlen(pid_string));
    close(pidfd);
  }

  /* create fifo.[pid] */
  char fifopid[PATH_MAX] = {0};
  sprintf(fifopid, "%s/fifo.%d", tmp->arg.run_path, getpid());
  int m = mkfifo(fifopid, 0644);
  if (m != 0) {
    perror("mkfifo fail!\n");
  }
  /* store TARGET FIFO (to be deleted) */
  strncpy(TARGET_FIFO, fifopid, strlen(fifopid));
...
int csiebox_server_run(csiebox_server* server) {
  ...
  // /* create fifo.[pid] */
  // char fifopid[PATH_MAX] = {0};
  // sprintf(fifopid, "%s/fifo.%d", server->arg.run_path, getpid());
  // int m = mkfifo(fifopid, 0644);
  // if (m != 0) {
  //   perror("mkfifo fail!\n");
  // }
...
static int parse_arg(csiebox_server* server, int argc, char** argv) {
  ...
  /* if -d is specified, set daemonize flag */
